...
/* Global Variable */
const char *key_words[]={"开始", "停", "左", "右","启动","停止","加速","减速" };
#define NUM_NAMES 6 // 抽奖数量

const char* names[NUM_NAMES] ={  // 抽奖名字
        "(^_^)",
        "(X_X)",
        "(-_-)?",
       "(TAT)",
...
ALIGN(RT_ALIGN_SIZE)
static char rng_stack[2048];
static struct rt_thread rng_thread;
...
void rng()
{
    ...
    {
        if (status == 1) {
            lcd_fill(0, 90, LCD_W, LCD_H,WHITE);
        }
        ...
        lcd_set_color(WHITE, BLUE);
        lcd_show_string(80,96,32,names[random%NUM_NAMES]);
        Hal_Delay_Ms(100);     //修改跳动时间。单位
    }
}
...
void lcd_display(void *parameter)
{
    ...
    /* 黑线 */
    lcd_fill(0,63,LCD_W,65,BLACK);

    lcd_show_string(115,120,24,"READY");
    ...
    while(1)
    {
        if (rt_mb_recv(&display_mb, (rt_ubase_t *)&result, RT_WAITING_FOREVER) == RT_EOK)
        {
            ...
            else
            {
                ...
                {
                    case up:
                        lcd_show_string(90,7+24,24,"result:Start");
...
//                        lcd_fill(x1,y1,x2,y2,RED);
                        ...
                        break;
                    case down:
                        lcd_show_string(90,7+24,24,"result:Stop");
                        lcd_fill(x1,y1,x2,y2,WHITE);
//                        y1+=20;
//                        if(y1>220) y1=75;
//                        x2=x1+10;
...
//                        break;
//                    case on:
//                        lcd_show_string(90,7+24,24,"led on");
//                        bledata[0]=0xaa;
//                        bledata[1]=0x55;
//                        bledata[2]=0x01;
//                        bledata[3]=0x01;
...
//                        break;
//                    case off:
//                        lcd_show_string(90,7+24,24,"led off");
//                        bledata[0]=0xaa;
//                        bledata[1]=0x55;
//                        bledata[2]=0x01;
//                        bledata[3]=0x00;
