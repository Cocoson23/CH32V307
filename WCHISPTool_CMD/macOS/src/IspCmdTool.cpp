...
{
    char  PortName[64];            /* usb or uart device name */
...
/* devices array */
Isp55xDevInforS AfxDnDev[DEV_MAX_NUMBER];
/* download option, must be set before operation */
CH55xIspOptionS IspOption;
/* device index */
...
int GetDevIndex()
{
    ...
    for (i = 0; i < iDevCnt; i++) {
        if ( IspOption.IspInterface == 1) {
            if (strstr(AfxDnDev[i].PortName, szStr)) {
                uDevIndex = i;
                isFind = true;
                goto _END;
            }
...
int main(int argc, char * argv[]) {
    ...
    sprintf(TextBufferTmp, "p:%s\r\nb:%d\r\nv:%d\r\nc:%s\r\no:%d\r\nf:%s\r\n",
        cmdPara.PortName, cmdPara.PortBaud, cmdPara.bPrtVer, cmdPara.CfgFile, cmdPara.OptType, cmdPara.FlashFile);
    std::cout << TextBufferTmp << std::endl;
...
retry:
    ...
    if (cmdPara.OptType == 0 && bRemove == FALSE) {
        if (WCH55x_IspRemoveProtect(uDevIndex)) {
            if (IspOption.IspInterface == 0) {
                wch55xusb_exit();
                sleep(2);
                bRemove = TRUE;
                wch55xusb_int();
